...
function* uploadFeature_() {
    ...
    try {
        ...
        if (folderStroe.get("traversed")) {
            images = JSON.parse(folderStroe.get(pathString) || [])
            yield put(actions.upload['success']({
                traversing: false,
            ...
            // 遍历目录和入库同时进行
            yield fork(walkDir, upload.path, pathString)
            yield  delay(1000)
        }
        ...
        for (var j = failureNumber + savedNumber; j < images.length; j++) {
...
function* walkDir(dir,pathString) {
